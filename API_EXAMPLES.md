# Примеры использования API (RU)

Ниже — минимальные примеры запросов к REST API панели: как получить JWT‑токен, создать клиента, получить его QR‑код и т.п. Все примеры по умолчанию используют `http://localhost:8082`, в продакшене подставьте свой домен/IP.

Оригинальный английский текст оставлен после русской секции.
...
Переменная `token` далее передаётся в заголовке:

```bash
TOKEN="полученный-токен"
curl -H "Authorization: Bearer $TOKEN" http://localhost:8082/api/servers
```

## Создание клиента и получение QR‑кода
...
В ответе поле `qr_code` содержит data URI, который можно сразу использовать в `<img src="...">` или сохранить в файл.
