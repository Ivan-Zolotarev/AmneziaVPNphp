# Руководство по настройке LDAP/AD (RU)

## Общее описание

Эта инструкция описывает, как подключить панель Amnezia VPN к внешнему LDAP/Active Directory для авторизации и управления пользователями.

- Поддерживаются OpenLDAP, Active Directory, FreeIPA и любые совместимые с RFC 4511 серверы.
- Панель использует LDAP для проверки логина/пароля и синхронизации данных пользователей в свою БД.

Основные шаги:

1. Включить LDAP в панели администрирования (**Настройки → LDAP**).
2. Заполнить параметры подключения (host, port, Base DN, Bind DN, фильтры поиска).
3. Настроить сопоставление групп LDAP ↔ ролей панели (admin/manager/viewer).
4. Проверить соединение и включить периодическую синхронизацию.

Ниже приведена оригинальная англоязычная инструкция с примерами конфигов и отладочными командами.
